function J = jacobian_matrix(q)
    ...
    x_rel = RobotParameters.b_b + L1*cos(theta1) + L2*cos(theta1+theta2);
    y_rel = L1*sin(theta1) + L2*sin(theta1+theta2);
    J(:,3) = [-sin(theta_b)*x_rel - cos(theta_b)*y_rel;
               cos(theta_b)*x_rel - sin(theta_b)*y_rel];
    J(:,4:5) = R * J_rel;
end
